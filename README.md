...
这是一个用于学习 Python 项目拆分、规则分析和大模型 API 调用的 AI 应用项目。

项目从 JSON 或 Excel 文件读取客户资料，先通过本地演示规则生成初步分析结果，再调用 DeepSeek API 生成自然语言客户分析报告，最终将结果保存为 JSON 文件。

本项目用于学习和演示，不是企业级成熟产品。代码中的客户分层阈值、数字化成熟度判断和系统推荐规则不代表正式行业标准，需要在真实业务中结合实际情况调整。

## 项目功能

- 支持读取 JSON 和 Excel 格式的客户数据
- 提取客户名称、行业、员工人数、当前痛点和现有系统
- 基于本地规则分析：
  - 客户等级
  - 推荐版本
  - 数字化成熟度
...
- 将数据读取、规则分析、Prompt 构造、API 调用和报告写入拆分为独立模块
- 同时保留结构化规则结果和自然语言 AI 报告
- 支持 JSON 与 Excel 两种常见输入格式
- API Key 通过环境变量读取，避免将密钥直接写入代码
- 规则和 Prompt 集中管理，便于学习不同模块在 AI 应用中的职责

## 后续优化方向

- 增加输入字段完整性和数据类型校验
- 完善空 Excel、异常表头和接口返回格式异常的处理
- 将客户分层阈值和行业规则移到独立配置文件
- 增加 API 请求超时、重试和更完整的错误处理
- 支持自动加载本地 `.env` 配置
- 增加单元测试和更多输入样例
- 根据实际业务数据持续调整分析规则和 Prompt
